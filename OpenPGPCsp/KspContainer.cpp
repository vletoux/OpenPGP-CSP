...
KspEnumNCryptKeyName* KspContainer::BuildEnumData()
{
	DWORD dwError = 0;
							...
							__in    DWORD   dwFlags)
{
	...
	__try
	{
		...
		{
			dwError = NTE_INVALID_PARAMETER;
			Trace(TRACE_LEVEL_ERROR, L"pszScope NULL");
			__leave;
		}
		if (!ppKeyName)
		{
		...
		if (!*ppEnumState)
		{
			*ppEnumState = KspContainer::BuildEnumData();
			if (!*ppEnumState )
			{
				dwError = GetLastError();
				__leave;
		...
		size_t size = sizeof(NCryptKeyName) + keynamesize;
...
KspKey* KspKey::LoadKey(KspContainer* kspcontainer, __in PCWSTR szReaderGiven, __in PCWSTR szKeyName)
{
	BOOL fReturn = FALSE;
	DWORD dwError = 0;
	KspKey* key = NULL;
	...
	__try
	{
		...
		if (szReaderGiven)
		{
			...
			if (!key->m_Card->LocateContainer(szContainer, &key->m_dwCardContainerId))
			{
				dwError = GetLastError();
		...
		if (!key->m_Card->GetPublicKey(key->m_dwCardContainerId, &pPubKey, &dwSize))
		{
			dwError = GetLastError();
		...
		key->m_dwLegacyKeySpec = (((BLOBHEADER*) pPubKey)->aiKeyAlg == CALG_RSA_KEYX ? AT_KEYEXCHANGE : AT_SIGNATURE);
		...
		dwError = BCryptImportKeyPair(key->m_hAlgProv,NULL,BCRYPT_RSAPUBLIC_BLOB,&key->m_key,(PUCHAR) pBcryptPublicKey, dwBcryptPublicKeySize,0);
		if (dwError)
		{
...
#pragma warning(suppress: 6386)
		...
		{
			dwError = BCryptGetProperty( m_key, NCRYPT_BLOCK_LENGTH_PROPERTY, pbOutput, cbOutput, pcbResult, dwFlags);
			if (dwError)
			{
		...
		{
			dwError = BCryptGetProperty( m_key, NCRYPT_LENGTH_PROPERTY, pbOutput, cbOutput, pcbResult, dwFlags);
			if (dwError)
			{
				Trace(TRACE_LEVEL_ERROR, L"BCryptGetProperty failed 0x%08X", dwError);
				__leave;
		...
		{
			dwError = BCryptGetProperty( m_key, NCRYPT_LENGTHS_PROPERTY, pbOutput, cbOutput, pcbResult, dwFlags);
			if (dwError)
			{
				Trace(TRACE_LEVEL_ERROR, L"BCryptGetProperty failed 0x%08X", dwError);
				__leave;
...
__in    DWORD   dwFlags)
{
	DWORD dwError = BCryptVerifySignature(m_key, pPaddingInfo, pbHashValue, cbHashValue, pbSignature, cbSignature, dwFlags);
	if (dwError)
	{
