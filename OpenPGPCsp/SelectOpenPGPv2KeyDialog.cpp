...
SelectOpenPGPv2KeyDialog::SelectOpenPGPv2KeyDialog(__in ALG_ID Algid)
{
	m_Algid  = Algid;
}

SelectOpenPGPv2KeyDialog::~SelectOpenPGPv2KeyDialog()
{
...
INT_PTR CALLBACK SelectOpenPGPv2KeyDialog::_dialogProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    if (WM_INITDIALOG == msg) {
        ((SelectOpenPGPv2KeyDialog*)lp)->_hwnd = hwnd;
        SetWindowLongPtr(hwnd, GWLP_USERDATA, lp);
    }
	SelectOpenPGPv2KeyDialog* dlg = (SelectOpenPGPv2KeyDialog*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
INT_PTR SelectOpenPGPv2KeyDialog::DialogProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) 
	{
	case WM_INITDIALOG:
...
void SelectOpenPGPv2KeyDialog::CenterWindow() 
{
    RECT rc;
...
VOID SelectOpenPGPv2KeyDialog::InitDialog()
{
	CenterWindow();
